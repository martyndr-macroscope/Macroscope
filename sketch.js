...
// --- VISIBILITY DEBUG / RESET (copy-paste) -----------------------------

window.debugVisibilityGates = function debugVisibilityGates() {
  const n = (nodes?.length || 0) | 0;
  if (!n) { console.warn("No nodes."); return; }

  // mimic recomputeVisibility() conditions
  const aiLensOn  = !!(lenses && lenses.aiDocs);
  const dimLensOn = !!(lenses && lenses.hideNonDim);
  ...
  // focus nodes union (same idea as recomputeVisibility)
  let focusNodes = null;
  if (Array.isArray(dimTools)) {
    for (const d of dimTools) {
      if (!d || !d.focusOn || !d.nodes) continue;
      if (!focusNodes) focusNodes = new Set();
      if (typeof d.nodes.forEach === 'function') d.nodes.forEach(i => focusNodes.add((i|0)));
      else if (Array.isArray(d.nodes)) for (const i of d.nodes) focusNodes.add((i|0));
    }
  }
  if (focusedDimSet && focusedDimSet.size) {
    if (!focusNodes) focusNodes = new Set();
  ...
  const counts = {
    n,
    passDeg: 0, passYear: 0, passDim: 0, passOA: 0, passExt: 0, passViab: 0, passCluster: 0, passFocus: 0,
    visible: 0
  };
  ...
  for (let i = 0; i < n; i++) {
    ...
    const passViab = (typeof visScaleForNode === 'function') ? (visScaleForNode(i) > 0) : true;
    const passFocus = focusNodes ? focusNodes.has(i) : true;

    if (passDeg) counts.passDeg++;
    if (passYear) counts.passYear++;
    if (passDim) counts.passDim++;
    if (passOA) counts.passOA++;
    if (passExt) counts.passExt++;
    if (passViab) counts.passViab++;
    if (passCluster) counts.passCluster++;
    if (passFocus) counts.passFocus++;

    if (passDeg && passYear && passDim && passOA && passExt && passViab && passCluster && passFocus) counts.visible++;
  }

  console.table(counts);
  console.log("Visibility sliders:", { visAllPubs, visDims, visAIDims, visEdges });
  console.log("Thresholds:", { degThreshold, extCitesThreshold, clusterSizeThreshold, yearLo, yearHi });
  console.log("Lenses:", { aiDocs: lenses?.aiDocs, hideNonDim: lenses?.hideNonDim, openAccess: lenses?.openAccess });
  console.log("Focus:", { focusedDimSetSize: focusedDimSet?.size || 0, anyDimFocusOn: (dimTools||[]).some(d=>d?.focusOn) });
};

window.hardResetVisibility = function hardResetVisibility() {
  // 1) Make sliders non-zero (this fixes passViab -> visScaleForNode > 0)
  visAllPubs = 1; visDims = 1; visAIDims = 1; visEdges = 1;

  // 2) Turn off restrictive lenses
  if (lenses) {
    lenses.hideNonDim = false;
    lenses.aiDocs = false;
    lenses.openAccess = false;
  }

  // 3) Clear focus filtering
  try { focusedDimSet?.clear?.(); } catch {}
  if (Array.isArray(dimTools)) for (const d of dimTools) if (d) d.focusOn = false;
  ...
  // 5) Recompute everything + repaint
  try { onGraphDataChanged?.(); } catch {}
  try { recomputeVisibility?.(); } catch {}
  try { updateInfo?.(); } catch {}
  try { redraw?.(); } catch {}
  try { fitContentInView?.(60); } catch {}
  console.log("hardResetVisibility(): done");
};